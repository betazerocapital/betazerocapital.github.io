...
var M = {
  ...
  createCode: function () {
    ...
    if (column) {
      var codeLength = M.randomFromInterval(
        M.settings.CODE_LENGTH_PARAMS.min,
        M.settings.CODE_LENGTH_PARAMS.max
      );
      var codeVelocity =
        Math.random() *
          (M.settings.VELOCITY_PARAMS.max - M.settings.VELOCITY_PARAMS.min) +
        M.settings.VELOCITY_PARAMS.min;
      ...
      M.codes[column][0].strength =
        M.codes[column][0].velocity / M.settings.VELOCITY_PARAMS.max;
